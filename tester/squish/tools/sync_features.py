...
SPECS_ROOT = os.path.join(REPO_ROOT, "feature-specs")
...
def main():
    for test_case, spec in sorted(BDD_TEST_CASES.items()):
        ...
        if not os.path.exists(source):
            raise SystemExit(
                "missing spec %s\n"
                "The Gherkin specs live in the 'feature-specs' submodule; run:\n"
                "  git submodule update --init feature-specs" % source)
