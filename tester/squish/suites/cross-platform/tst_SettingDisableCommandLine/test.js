...
function main(){
    ...
    App.withInstance(shown, function(){
        ...
        openCallSettings();
        test.verify(Settings.shows(CTX, ANCHOR), "call settings opened (shown)");
        test.verify(Settings.shows(CTX, FEATURE), "Command line shown when disable_command_line=0");
    });
    App.withInstance(hidden, function(){
        test.verify(MainPage.waitLoaded(), "main loaded (command line hidden)");
        openCallSettings();
        test.verify(Settings.shows(CTX, ANCHOR), "call settings opened (hidden)");
        test.verify(Settings.hides(CTX, FEATURE), "Command line hidden when disable_command_line=1");
    });
}
