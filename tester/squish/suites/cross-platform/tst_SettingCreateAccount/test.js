...
function main(){
    ...
    App.withInstance(shown, function(){
        ...
        test.verify(Settings.shows(FEATURE), "create-account shown when assistant_hide_create_account=0");
    });
    App.withInstance(hidden, function(){
        WelcomePage.skipIfPresent();
        test.verify(Settings.shows(ANCHOR), "login page reached");
        test.verify(Settings.hides(FEATURE), "create-account hidden when assistant_hide_create_account=1");
    });
}
